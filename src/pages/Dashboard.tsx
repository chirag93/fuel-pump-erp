...
import { Bar, BarChart, XAxis, YAxis, Tooltip, Line, LineChart, ResponsiveContainer } from 'recharts';
...
const Dashboard = () => {
  ...
  const formattedStartDate = useMemo(() => dateRange.from ? format(dateRange.from, 'yyyy-MM-dd') : format(startOfMonth(new Date()), 'yyyy-MM-dd'), [dateRange.from]);
  const formattedEndDate = useMemo(() => dateRange.to ? format(dateRange.to, 'yyyy-MM-dd') : format(new Date(), 'yyyy-MM-dd'), [dateRange.to]);
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('customers').select('balance').eq('fuel_pump_id', fuelPumpId);
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('transactions').select('date, amount, fuel_type').eq('fuel_pump_id', fuelPumpId).gte('date', formattedStartDate).lte('date', formattedEndDate).order('date', {
        ascending: true
      });
      if (error) throw error;
      if (!data || data.length === 0) {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('daily_readings').select('date, fuel_type, sales_per_tank_stock').eq('fuel_pump_id', fuelPumpId).gte('date', formattedStartDate).lte('date', formattedEndDate).order('date', {
        ascending: true
      });
      if (error) throw error;
      if (!data || data.length === 0) {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('transactions').select('id, fuel_type, amount, created_at, quantity').eq('fuel_pump_id', fuelPumpId).order('created_at', {
        ascending: false
      }).limit(3);
      if (error) throw error;
      return data || [];
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('fuel_settings').select('fuel_type, current_level, tank_capacity').eq('fuel_pump_id', fuelPumpId);
      if (error) throw error;
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data: salesData,
        error: salesError
      } = await supabase.from('transactions').select('amount').eq('fuel_pump_id', fuelPumpId).gte('date', formattedStartDate).lte('date', formattedEndDate);
      if (salesError) throw salesError;

      // Get total sales amount for the previous period of same length
      const prevPeriodDays = dateRange.from && dateRange.to ? Math.ceil((dateRange.to.getTime() - dateRange.from.getTime()) / (1000 * 60 * 60 * 24)) : 30;
      ...
      const {
        data: prevSalesData,
        error: prevSalesError
      } = await supabase.from('transactions').select('amount').eq('fuel_pump_id', fuelPumpId).gte('date', prevStartDate).lte('date', prevEndDate);
      ...
      const {
        data: fuelData,
        error: fuelError
      } = await supabase.from('transactions').select('quantity').eq('fuel_pump_id', fuelPumpId).gte('date', formattedStartDate).lte('date', formattedEndDate);
  ...
  const isLoading = isLoadingSales || isLoadingFuel || isLoadingTransactions || isLoadingFuelLevels || isLoadingMetrics || isLoadingCredits;
        ...
        <div className="flex items-center space-x-2">
          <Popover open={datePickerOpen} onOpenChange={setDatePickerOpen}>
            <PopoverTrigger asChild>
              <Button variant="outline" className="w-[240px] justify-start text-left font-normal">
                ...
                {dateRange.from ? dateRange.to ? <>
                    ...
                    </> : format(dateRange.from, "MMM d, yyyy") : <span>Pick a date range</span>}
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0" align="end">
              ...
              <CalendarComponent initialFocus mode="range" defaultMonth={dateRange.from} selected={dateRange} onSelect={handleDateRangeSelect} numberOfMonths={2} className={cn("p-3 pointer-events-auto")} />
            </PopoverContent>
          </Popover>
            ...
            <TabsContent value="overview" className="space-y-4">
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
                ...
                <Card className="col-span-3">
                  ...
                  <CardContent className="pl-2">
                    {fuelData.length > 0 ? <ResponsiveContainer width="100%" height={300}>
                        <LineChart data={fuelData}>
                          <XAxis dataKey="name" fontSize={12} tickLine={false} axisLine={false} />
                          <YAxis fontSize={12} tickLine={false} axisLine={false} tickFormatter={value => `${value}L`} />
                          <Tooltip formatter={value => [`${value}L`, 'Volume']} />
                          <Line type="monotone" dataKey="petrol" stroke="#FF6B35" strokeWidth={2} dot={{
                      r: 4
                    }} activeDot={{
                      r: 6
                    }} />
                          <Line type="monotone" dataKey="diesel" stroke="#004E89" strokeWidth={2} dot={{
                      r: 4
                    }} activeDot={{
                      r: 6
                    }} />
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Card>
                  ...
                  <CardContent>
                    {recentTransactions.length > 0 ? <div className="space-y-4">
                        {recentTransactions.map(transaction => <div key={transaction.id} className="flex items-center gap-4 rounded-lg border p-3">
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {Object.keys(fuelLevels).length > 0 ? Object.entries(fuelLevels).map(([fuelType, percentage]) => <div key={fuelType} className="space-y-2">
                            <div className="flex items-center justify-between text-sm">
                              <p>{fuelType}</p>
                              <p className="font-medium">{percentage}%</p>
                            </div>
                            <div className="h-2 rounded-full bg-muted">
                              <div className={`h-full rounded-full ${fuelType.toLowerCase().includes('petrol') ? 'bg-primary' : fuelType.toLowerCase().includes('diesel') ? 'bg-blue-600' : 'bg-amber-500'}`} style={{
                        width: `${percentage}%`
                      }} />
                            </div>
                          </div>) : <div className="flex items-center justify-center h-32 text-muted-foreground">
